...
// Helper: apply CTRX DMUX configuration for a selected device
static error_t apply_device_dmux(uint8_t device, IfxRfe_demoConfigParams_t *cfg)
{
    // Configure Aurix-side GPIO flags for DMUX pins. Adjust flags if you need different pull/drive.
    IfxRfe_dmuxFlags_t aurixFlags = {
        .dmux1Flags = GPIO_FLAG_OUTPUT_DRIVE_HIGH | GPIO_FLAG_PULL_DOWN,
        .dmux2Flags = GPIO_FLAG_OUTPUT_DRIVE_HIGH | GPIO_FLAG_PULL_DOWN,
        .dmux3Flags = 0};

    EXIT_ON_IFXRFE_ERROR(IfxRfe_selectDevice(device));

    // Safe configure will check for contention between Aurix and CTRX pin directions
    EXIT_ON_IFXRFE_ERROR(IfxRfe_safeConfigureDmux(cfg->ctrxDmux, aurixFlags));
...
int main(void)
{
    ...
    // Apply DMUX config to both CTRX devices (primary + secondary)
    printf("Applying DMUX configuration to both CTRX devices...\n");
    EXIT_ON_IFXRFE_ERROR(apply_device_dmux(DEVICE_PRIMARY, &configParamsPrim));
    EXIT_ON_IFXRFE_ERROR(apply_device_dmux(DEVICE_SECONDARY, &configParamsSec));
