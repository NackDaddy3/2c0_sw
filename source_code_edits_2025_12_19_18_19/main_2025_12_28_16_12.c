...
#define NUM_DEV 2
// #define NUM_RAMPS_TOTAL NUM_RAMPS_DEV*NUM_DEV //No, the *2 is counted in the num Rx
#define FPS 30
//TODO: Ask Heinz 
#define NUM_SAMPS_PER_RX 1024 //CFG A and B
// #define NUM_SAMPS_PER_RX 1536 //CFG C
#include <unistd.h>   // access()

//AG: For CFG Selections
#define CFG_SEL "CFG_B"


//AG: For DMUX Latch
#define DMUX_LATCH_SAMPLE_US   200    // 50–200us is a good start
#define DMUX_LATCH_WINDOW_US   FINISH_RAMP_DELAY_US  // sample during the ramp window (50ms)
...
// static volatile int g_running = 0;   // 0 = idle/low power, 1 = streaming
static volatile int g_running = 1;   // 0 = idle/low power, 1 = streaming
static char g_config[32] = CFG_SEL;  // for later; keep placeholder now
...
// static void latch_dmux_activity(uint16_t gpio_id,
//                                 uint32_t window_us,
//                                 uint32_t sample_us,
//                                 const char *tag)
// {
...
//     int64_t t_start = PlatformTime_now();
//     int64_t t_first_high = -1;
//     int64_t t_last_high = -1;

//     // Initial read
//     (void)PlatformGpio_get(gpio_id, &prev);

//     // Sample for window_us total
//     uint32_t elapsed_us = 0;
//     while (elapsed_us < window_us)
//     {
//         (void)PlatformGpio_get(gpio_id, &v);
//         samples++;
...
//             {
//                 seen_high = true;
//                 t_first_high = PlatformTime_now();
//             }
//             t_last_high = PlatformTime_now();
//         }
...
//     int64_t t_end = PlatformTime_now();

//     printf("[DMUX LATCH] %s: window=%u us sample=%u us | samples=%u high_samples=%u transitions=%u seen_high=%s | t_start=%" PRId64 " t_end=%" PRId64,
//            tag, window_us, sample_us, samples, high_samples, transitions,
//            seen_high ? "YES" : "NO", t_start, t_end);

//     if (seen_high)
//     {
//         printf(" | first_high=%" PRId64 " last_high=%" PRId64, t_first_high, t_last_high);
//     }
//     printf("\n");
// }

//AG: change time stamps to durations
...
//     //AG: 
//     int64_t t_duration_first = -1;
//     int64_t t_duration_last = -1;
//     int64_t t_duration_total= -1;
...
typedef struct {
    ...
    int64_t first_high_ts;
    int64_t last_high_ts;
} dmux_latch_t;

static dmux_latch_t dmux_latch;


static void dmux_latch_start(uint16_t gpio_id)
{
    bool v = false;

    memset(&dmux_latch, 0, sizeof(dmux_latch));
    dmux_latch.first_high_ts = -1;
    dmux_latch.last_high_ts  = -1;

    PlatformGpio_get(gpio_id, &v);
    dmux_latch.prev = v;
}

static inline void dmux_latch_tick(uint16_t gpio_id)
// static inline void dmux_latch_tick(uint16_t gpio_id, IfxRfe_setSignals_t sig_req_hi)
{
    bool v;
    ...
    if (v)
    {
        if (!dmux_latch.seen_high)
            dmux_latch.first_high_ts = PlatformTime_now();

        dmux_latch.seen_high = true;
        dmux_latch.last_high_ts = PlatformTime_now();
    }

    if (v != dmux_latch.prev)
    {
        dmux_latch.transitions++;
        dmux_latch.prev = v;
    }
}

static void dmux_latch_stop(const char *tag)
{
    printf("[DMUX LATCH] %s: samples=%u transitions=%u seen_high=%s",
           tag,
           dmux_latch.samples,
           dmux_latch.transitions,
           dmux_latch.seen_high ? "YES" : "NO");

    if (dmux_latch.seen_high)
    {
        printf(" first_high=%" PRId64 " last_high=%" PRId64,
               dmux_latch.first_high_ts,
               dmux_latch.last_high_ts);
    }
 ...
 *                                         
 */
//AG: Add READER functions


// static void cmd_fifo_init(void)
// {
//     const char *fifo = "/tmp/radar_cmd";

//     // Create FIFO if missing (ignore "already exists")
//     if (mkfifo(fifo, 0666) < 0 && errno != EEXIST) {
//         perror("mkfifo(/tmp/radar_cmd)");
//         // Not fatal; but commands won't work
//         return;
//     }

//     // Open RDWR to prevent EOF when writer closes; nonblocking so we can poll
//     g_cmd_fd = open(fifo, O_RDWR | O_NONBLOCK);
//     if (g_cmd_fd < 0) {
//         perror("open(/tmp/radar_cmd)");
//     }
// }

// // returns 1 if it consumed a STOP or START that changes state
// static int cmd_fifo_poll(void)
// {
//     if (g_cmd_fd < 0) return 0;

//     char buf[256];
//     ssize_t n = read(g_cmd_fd, buf, sizeof(buf) - 1);
//     if (n < 0) {
//         if (errno == EAGAIN || errno == EWOULDBLOCK) return 0; // nothing there
//         perror("read(/tmp/radar_cmd)");
//         return 0;
//     }
//     if (n == 0) {
//         // With O_RDWR this is unlikely. If it happens, just ignore.
//         return 0;
//     }

//     buf[n] = '\0';

//     // FIFO might contain multiple commands; simplest: scan keywords
//     if (strstr(buf, "STOP")) {
//         g_running = 0;
//         return 1;
//     }
...
//     // We'll add SET_CONFIG later
//     // if (strstr(buf, "SET_CONFIG")) { ... }
...
//AG: READER second method:
static int stop_requested(void)
{
    return (access("/tmp/radar_stop", F_OK) == 0);
}

static void clear_stop_flag(void)
{
    unlink("/tmp/radar_stop");
}
...
int main(void)
{
    ...
    //AG: Setting for the pin to be an input on Orin side
    EXIT_ON_IFXRFE_ERROR(PlatformGpio_configure(GPIO_ID_DMUX1_A, GPIO_FLAG_INPUT_ENABLE | GPIO_FLAG_PULL_DOWN));  //change the flag to an input 
// EXIT_ON_IFXRFE_ERROR(PlatformGpio_configure(GPIO_ID_DMUX1_A, GPIO_FLAG_INPUT_ENABLE | GPIO_FLAG_PULL_UP));  //change the flag to an input 


/**
 *   ___           _              _   ___                          
 *  | _ \__ _ _  _| |___  __ _ __| | | _ \__ _ _ _ __ _ _ __  ___  
 *  |  _/ _` | || | / _ \/ _` / _` | |  _/ _` | '_/ _` | '  \(_-<  
 *  |_| \__,_|\_, |_\___/\__,_\__,_| |_| \__,_|_| \__,_|_|_|_/__/  
 *            |__/                                                 
  */
 
// /* Snapshot of parameters used to compute radar ADC payload */
// typedef struct {
//     /* Topology / data shape */
//     uint32_t num_rx;           // total active RX channels
//     uint32_t num_ramps;        // chirps per cycle across all devices (or per device if that’s your convention)
//     uint32_t num_samples;      // samples per ramp (post-decimation)
//     uint32_t sample_time_us;   // ramp sample time (µs) if you prefer time-based sampling checks
//     uint32_t bit_width;        // RAW10/12/14 -> 10/12/14

//     /* Rates */
//     uint32_t deci_rate_meg;    // DFE output sample rate in MS/s (e.g., 50 → 50e6 samples/s)
//     uint32_t fps;              // ramp scenarios per second

//     /* Timestamps (µs) — choose wide type to avoid overflow */
//     uint64_t measure_start;    // t_start (µs, system clock)
//     uint64_t measure_end;      // t_end   (µs, system clock)
//     uint64_t measure_total;    // t_total (µs, derived or captured)

//     /* Derived cached values (optional convenience) */
//     uint64_t samples_per_cycle;  // num_rx * num_ramps * num_samples
//     double   bits_per_cycle;     // samples_per_cycle * bit_width
//     double   bps;                // bits per second (approx, using fps)
// } payload_params_t;
...
// --- ADC payload snapshot: create once, fill progressively ---
payload_params_t payload;
ace_payload_clear(&payload);      // zero all fields

//AG: let's set from our defines first 
payload.num_rx = NUM_RX;
payload.num_tx = NUM_TX;
//TODO: get this from Sequencer? low priority
// payload.num_ramps = NUM_RAMPS_DEV;
payload.fps = FPS;
//AG: still no idea where this comes from so lets just make it up
payload.num_samples = NUM_SAMPS_PER_RX;

payload.cycle_count = 0;
payload.cycle_max = NUM_ITERATIONS; 
...
//TODO: Set all the parameters
    ...
    IfxRfe_gpioDefinitions_t gpios[CTRX_DEV_COUNT] = {
        {.spiRftId = GPIO_ID_RFT_A,  // Primary device RFT pin
         ...
         //AG: try unused and see what happens
// .dmux1Id  = GPIO_UNUSED,
         //AG:original
         .dmux2Id  = GPIO_UNUSED,
         .dmux3Id  = GPIO_UNUSED},
         //AG: try this
// .dmux2Id  = GPIO_ID_DMUX1_A,
// .dmux3Id  = GPIO_ID_DMUX1_A},
...
// Inspect NUM_LOOPS for the sequencer program you are about to use
int32_t numLoopsAce = parse_first_num_loops(seqData_ace,
                             sizeof(seqData_ace) / sizeof(uint32_t));
if (numLoopsAce < 0) {
    printf("CTRX_A: No OP_LOOP found in sequencer program.\n");
} else {
    printf("CTRX_A: First OP_LOOP NUM_LOOPS = %d\n", numLoopsAce);
    // Optional: assert or warn if unexpected; 0 can mean 'infinite' per setup
}

payload.num_ramps = numLoopsAce;
        
    //AG:original
    // EXIT_ON_IFXRFE_ERROR(IfxRfe_loadSequencerData(0, seqData_prim, sizeof(seqData_prim) / sizeof(uint32_t)));
//AG: custom
EXIT_ON_IFXRFE_ERROR(IfxRfe_loadSequencerData(0, seqData_ace, sizeof(seqData_ace) / sizeof(uint32_t)));
    ...
    printf("Ramp params configured for CTRX_A (Primary)\n");


//AG: Configure DMUX Primary
EXIT_ON_IFXRFE_ERROR(IfxRfe_configureDmux(configParamsPrim.ctrxDmux));
EXIT_ON_IFXRFE_ERROR(IfxRfe_configureDios(configParamsPrim.dios));
printf("DMUX and DIOS configured for CTRX_A (Primary)\n");
 ...
 */
    ...
    //AG: Parser:
// Inspect NUM_LOOPS for the sequencer program you are about to use
int32_t numLoopsDeuce = parse_first_num_loops(seqData_deuce,
                             sizeof(seqData_deuce) / sizeof(uint32_t));
if (numLoopsDeuce < 0) {
    printf("CTRX_B: No OP_LOOP found in sequencer program.\n");
} else {
    printf("CTRX_B: First OP_LOOP NUM_LOOPS = %d\n", numLoopsDeuce);
    // Optional: assert or warn if unexpected; 0 can mean 'infinite' per setup
}
    ...
    EXIT_ON_IFXRFE_ERROR(IfxRfe_loadSequencerData(0, seqData_deuce, sizeof(seqData_deuce) / sizeof(uint32_t)));
...
//AG: Configure DMUX Secondary
EXIT_ON_IFXRFE_ERROR(IfxRfe_configureDmux(configParamsSec.ctrxDmux));
EXIT_ON_IFXRFE_ERROR(IfxRfe_configureDios(configParamsSec.dios));

//TODO: Safe DMUX Config





//AG: Add init fifo for READER
// cmd_fifo_init();
    ...
    IfxRfe_setSignals_t test_sig = { .signal_bit_mask = 0x01, .signal_bit_value = 0x01 }; // mask: GP1, value: 1
// IfxRfe_setSignals_t sig_req_hi
// EXIT_ON_IFXRFE_ERROR(IfxRfe_setSignals(sig_req_hi));         // toggles GP1 high
EXIT_ON_IFXRFE_ERROR(IfxRfe_setSignals(test_sig));         // toggles GP1 high
bool gpio_1_state;
PlatformGpio_get(GPIO_ID_DMUX1_A, &gpio_1_state);
printf("[DMUX TEST] GP1 HIGH, Orin reads: %s\n", gpio_1_state ? "HIGH" : "LOW");
    
printf("\n");

    // test_sig = { .signal_bit_mask = 0x01, .signal_bit_value = 0x00 }; // mask: GP1, value: 0
    test_sig.signal_bit_value = 0x00;
// IfxRfe_setSignals_t sig_req_hi
EXIT_ON_IFXRFE_ERROR(IfxRfe_setSignals(test_sig));         // toggles GP1 low
PlatformGpio_get(GPIO_ID_DMUX1_A, &gpio_1_state);
printf("[DMUX TEST] GP1 HIGH, Orin reads: %s\n", gpio_1_state ? "HIGH" : "LOW");
...
//TODO: Actually calculate this from NTIME*(5us) * decimation factor
// uint32_t num_samples_per_ramp = 1024; //Just going to try 1024 
uint32_t num_samples_per_ramp = 1536; //1536

uint8_t bits_per_sample = configParamsPrim.rxcfg.data_width;
//AG: 
payload.bit_width = getDataWidth(configParamsPrim.rxcfg.data_width);

payload.csi2_rate_mbps = getCsi2RateMbps(configParamsPrim.rxcfg.hsrif_csi2_data_rate);

// uint32_t IQ_mult = 2;
uint32_t IQ_mult = 1; //per INF recommendation
uint32_t fps = FPS;
...
// static inline double IfxRfe_getSampleRateMSps(uint8_t sel)
// {
//     switch (sel)
//     {
//         case DecSel_50MS:    return 50.0;
//         case DecSel_33p33MS: return 33.33;
//         case DecSel_25MS:    return 25.0;
//         case DecSel_20MS:    return 20.0;
//         case DecSel_16p67MS: return 16.67;
//         case DecSel_12p5MS:  return 12.5;
//         case DecSel_10MS:    return 10.0;
//         case DecSel_8p33MS:  return 8.33;
//         case DecSel_6p25MS:  return 6.25;
//         case DecSel_5MS:     return 5.0;
//         default:             return -1.0;
//     }
// }

payload.deci_rate_meg  = getSampleRateMSps(configParamsPrim.rxcfg.dec_sel);


// float fdec_freq = 50000.0;
float fdec_freq = 50.0;


// float fcalc_num_samples;
// float fpayload;
// float fgigapayload;

double fcalc_num_samples;
double fpayload;
double fgigapayload;


// uint64_t radar_payload;
double radar_payload;
uint64_t giga_radar_payload;
uint64_t gigabit = 1000000000;



//AG: start timing variables, they're probably in clock cycles?
// int64_t start_prim = PlatformTime_now();
// // printf("prim: %" PRIu64 " cycles\n", start_prim);
...
// int64_t cycle_prim;
// int64_t cycle_sec = PlatformTime_now();;
// // printf("total: %" PRIu64 " cycles\n", cycle_sec);

int64_t prim_start_begin_ramp;
int64_t prim_end_begin_ramp;
int64_t prim_start_finish_ramp;
int64_t prim_end_finish_ramp;

int64_t sec_start_begin_ramp;
int64_t sec_end_begin_ramp;
int64_t sec_start_finish_ramp;
int64_t sec_end_finish_ramp;


int64_t prim_cycle_begin_ramp;
int64_t prim_cycle_end_ramp;
int64_t prim_cycle_between_ramp;
int64_t prim_cycle_total_ramp;

int64_t sec_cycle_begin_ramp;
int64_t sec_cycle_end_ramp;
int64_t sec_cycle_between_ramp;
int64_t sec_cycle_total_ramp;

//AG: CGT
ui_write_radar_status("INACTIVE", 0.0,0,0, 0, 0,0.0, CFG_SEL,payload.cycle_count,payload.cycle_max);

//AG: let's calculate what we can before getting into the loop

payload.samples_per_cycle = payload.num_rx * payload.num_ramps * payload.num_samples;
payload.bits_per_cycle = payload.samples_per_cycle * payload.bit_width;
payload.bps = payload.bits_per_cycle * payload.fps;



/**
 *   ____      _    ____    _    ____    ____ _____  _    ____ _____ 
 *  |  _ \    / \  |  _ \  / \  |  _ \  / ___|_   _|/ \  |  _ \_   _|
 *  | |_) |  / _ \ | | | |/ _ \ | |_) | \___ \ | | / _ \ | |_) || |  
...
//AG: leave in idle running loop waiting for START command from UI
    // -------- IDLE: wait for START --------
//     while (!g_running) {
//         cmd_fifo_poll();                 // check if START arrived
//         usleep(100 * 1000);              // 100ms idle sleep (low CPU)

// //AG: Added
// static uint64_t last_idle_ms = 0;
// uint64_t now = ui_now_ms();
// if (now - last_idle_ms > 1000) {
//     ui_write_radar_status("INACTIVE", 0.0, payload.fps, payload.num_ramps, "CFG_A", payload.cycle_count, payload.cycle_max);
//     last_idle_ms = now;
// }
    ...
    // printf("START received -> streaming...\n");
    // payload.cycle_count = 0; // reset cycle count on START

//AG: make the running cycle 
// while (g_running)
//     {
        payload.cycle_count++;

        // Poll STOP early so you can stop quickly
        // cmd_fifo_poll();
        // if (!g_running) break;

        // ---- Paste your existing “one cycle” logic here ----
        // Everything currently inside:
        // for (int i=0; i<NUM_ITERATIONS; i++) { ... }
        // becomes "one cycle" in this while loop.
        //
        // IMPORTANT: remove references to loop index i if needed

        // After computing fgigapayload, write status:
        

        // Poll again at end of cycle too
        // cmd_fifo_poll();
...
//AG: Latch start 
// dmux_latch_start(GPIO_ID_DMUX1_A);   // <<< START LATCH HERE
// dmux_latch_start(GPIO_ID_OK);   // <<< START LATCH HERE

        // Start ramp scenario on both devices simultaneously
        //AG: Let's time it
        prim_start_begin_ramp = PlatformTime_now();
        EXIT_ON_IFXRFE_ERROR(IfxRfe_startRampScenario());  // Secondary (already selected)
        prim_end_begin_ramp = PlatformTime_now();

        EXIT_ON_IFXRFE_ERROR(IfxRfe_selectDevice(DEVICE_PRIMARY));

        sec_start_begin_ramp = PlatformTime_now();
        EXIT_ON_IFXRFE_ERROR(IfxRfe_startRampScenario());
        sec_end_begin_ramp = PlatformTime_now();
        
        //AG: original
        usleep(FINISH_RAMP_DELAY_US);  // Wait for ramp scenario to complete
        //AG: replace with Latch
        // latch_dmux_activity(GPIO_ID_DMUX1_A, DMUX_LATCH_WINDOW_US, DMUX_LATCH_SAMPLE_US, "during ramp window");


//AG: 
/* Latch ticks during wait */
// int elapsed = 0;
// while (elapsed < FINISH_RAMP_DELAY_US)
// {
// dmux_latch_tick(GPIO_ID_DMUX1_A);
// // dmux_latch_tick(GPIO_ID_DMUX1_A, test_sig);
// // dmux_latch_tick(GPIO_ID_OK);

// usleep(50);            // 50 µs sampling
// elapsed += 50;
// }
        ...
        prim_start_finish_ramp = PlatformTime_now();        
        EXIT_ON_IFXRFE_ERROR(IfxRfe_finishRampScenario(&frsres));
        prim_end_finish_ramp = PlatformTime_now();        

        EXIT_ON_IFXRFE_ERROR(IfxRfe_selectDevice(DEVICE_SECONDARY));

        sec_start_finish_ramp = PlatformTime_now();        
        EXIT_ON_IFXRFE_ERROR(IfxRfe_finishRampScenario(&frsres));
        sec_end_finish_ramp = PlatformTime_now();        

//AG: Latch End 
// dmux_latch_stop("full ramp window");   // <<< STOP LATCH HERE

//AG:



         prim_cycle_begin_ramp = prim_end_begin_ramp - prim_start_begin_ramp;
         prim_cycle_end_ramp = prim_end_finish_ramp - prim_start_finish_ramp;
         prim_cycle_between_ramp =  prim_start_finish_ramp - prim_end_begin_ramp;
         prim_cycle_total_ramp = prim_end_finish_ramp - prim_start_begin_ramp;


         sec_cycle_begin_ramp = sec_end_begin_ramp - sec_start_begin_ramp;
         sec_cycle_end_ramp = sec_end_finish_ramp - sec_start_finish_ramp;
         sec_cycle_between_ramp =  sec_start_finish_ramp - sec_end_begin_ramp;
         sec_cycle_total_ramp = sec_end_finish_ramp - sec_start_begin_ramp;


fcalc_num_samples = (float)prim_cycle_begin_ramp / fdec_freq;

        // //AG: do the calculation
        // cycle_prim = end_prim - start_prim;
        // cycle_sec = end_sec - start_sec;



     //AG:
        //Calculate payload:
// radar_payload = num_rx*num_ramps*num_samples_per_ramp*bits_per_sample*IQ_mult*fps;

// radar_payload = num_rx*num_ramps*(uint64_t)fcalc_num_samples*bits_per_sample*IQ_mult*fps;

// radar_payload = payload.num_rx*payload.num_ramps*(uint64_t)fcalc_num_samples*payload.bit_width*payload.fps;
// radar_payload = payload.num_rx*payload.num_ramps*payload.num_samples*payload.bit_width*payload.fps;

radar_payload = payload.bps;

// radar_payload = num_rx*num_ramps*(uint32_t)fcalc_num_samples*bits_per_sample*IQ_mult*fps;
// giga_radar_payload = radar_payload / gigabit;

fpayload = radar_payload;
fgigapayload = fpayload/1000000000;


//TODO: check out TOT_NUM_OF FRAMES or whatever
        //TODO: Add Timer 
        // printf("Radar cycle %d/%d\n", i + 1, NUM_ITERATIONS);
// printf("Radar cycle %d/%d with payload of uint64_t: % bps\n", i + 1, NUM_ITERATIONS, radar_payload);
// printf("Radar cycle %d/%d\n", i + 1, NUM_ITERATIONS);
// printf("GMSL payload of %" PRIu64 "\n", radar_payload);

// printf("\rGMSL payload of %          " PRIu64 , radar_payload);
printf("\rGMSL payload of %.5f     for %i cycles" , fgigapayload, payload.cycle_count);
// printf("\rGMSL payload of %.5f Gbps with %u Rx, %u ramps, %u samples per ramp, %u bits per sample, and %u fps" , fgigapayload, payload.num_rx, payload.num_ramps, payload.num_samples, payload.bit_width, payload.fps);
fflush(stdout);
// printf("GMSL payload of %" PRIu64 "\n", giga_radar_payload);
// printf("Primary CTRX cycle took %" PRIu64 "\n", cycle_prim);
// printf("Secondary CTRX cycle took %" PRIu64 "\n", cycle_sec);

//AG: Update UI
// ui_write_radar_status("ACTIVE", fgigapayload,payload.num_rx, payload.num_tx, (int)payload.fps, (int)payload.num_ramps, "CFG_C",payload.cycle_count,payload.cycle_max);
ui_write_radar_status("ACTIVE", fgigapayload,payload.num_rx, payload.num_tx, (int)payload.fps, (int)payload.num_ramps,payload.csi2_rate_mbps, CFG_SEL,payload.cycle_count,payload.cycle_max);
   



//printf("Primary start ramp scenario: %" PRId64 " Primary finish ramp scenario: %" PRId64 " Primary total ramp scenario: %" PRId64 " Primary between ramp scenario: %" PRId64 "\n", prim_cycle_begin_ramp, prim_cycle_end_ramp, prim_cycle_total_ramp, prim_cycle_between_ramp);

//printf("Secondary start ramp scenario: %" PRId64 " Secondary finish ramp scenario: %" PRId64 " Secondary total ramp scenario: %" PRId64 " Secondary between ramp scenario: %" PRId64 "\n", sec_cycle_begin_ramp, sec_cycle_end_ramp,sec_cycle_total_ramp, sec_cycle_between_ramp);
...
//  } // End bracket for the START loop
...
// cmd_fifo_poll();
    ...
    } // End for while(1) / while(g_running) loop / for loop
    ...
    printf("STOP received -> going low power...\n");
 ...
 *  | |_) |  / _ \ | | | |/ _ \ | |_) | |  _| |  \| | | | |
 *  |  _ <  / ___ \| |_| / ___ \|  _ <  | |___| |\  | |_| |
 *  |_| \_\/_/   \_\____/_/   \_\_| \_\ |_____|_| \_|____/ 
 *                                                         
 */
...
//AG: set to INACTIVE on GUI
ui_write_radar_status("INACTIVE", 0.0,(int)payload.num_rx, (int)payload.num_tx,(int)payload.fps, (int)payload.num_ramps,payload.csi2_rate_mbps, CFG_SEL,payload.cycle_count,payload.cycle_max);
...
// //AG: set to INACTIVE on GUI
// ui_write_radar_status("INACTIVE", 0.0, (int)payload.fps, (int)payload.num_ramps, "CFG_A",payload.cycle_count,payload.cycle_max);


    // Cleanup platform resources
    //AG: original
    cleanup_platform(); //AG: comment out so we can get to a START state? 

    printf("=== SUCCESS ===\n");
    return EXIT_SUCCESS;
    //AG: TODO: bring back continue?? 
    // continue; //AG from CGT

    //AG: Thought: its possible that helper thinks this is the end of the while loop and not the main loop. 
            //AG: ACTUALLY that's exactly what it thinks and possibly how we should implement this?
                //AG: the only thing is that we need to reconfig the entire thing after it goes into low power 
    //TODO: Fix this
        //AG: IDEA: STOP just kills the program, and START will start the demo again. This is our path of least resistance, we can be more dynamic with loops later.
