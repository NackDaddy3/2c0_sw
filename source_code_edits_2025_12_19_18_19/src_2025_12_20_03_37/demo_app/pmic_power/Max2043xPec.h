...
/// \retval IFXRFE_E_INVALID_PARAMETER if the provided voltage parameter has unsupported bits set (only the input and output voltages are allowed)
...
/// \retval IFXRFE_E_SUCCESS on success
/// \retval IFXRFE_E_UNEXPECTED_VALUE   if the provided millivolt value is less than the offset or
...
/// \retval IFXRFE_E_INVALID_PARAMETER if the provided output parameter does not match any of the expected voltage output definitions (MAX2043x_VOLTAGE_OUT2, MAX2043x_VOLTAGE_OUT4)
/// \note error code from \ref Wrapper_I2cRead on error
/// \note error code from \ref Wrapper_I2cWrite on error
...
/// \retval IFXRFE_E_SUCCESS on success
/// \retval IFXRFE_E_TIMEOUT if the state of the pin does not change to true within the specified timeout period
/// \note error code from \ref Max2043x_mapVoltageToReset on error
/// \note error code from \ref Max2043x_setVoltageOutput on error
/// \note error code from \ref Wrapper_GpioSet on error
/// \note error code from \ref Wrapper_GetDeadLine on error
/// \note error code from \ref Wrapper_Now on error
/// \note error code from \ref Wrapper_GpioGet on error
