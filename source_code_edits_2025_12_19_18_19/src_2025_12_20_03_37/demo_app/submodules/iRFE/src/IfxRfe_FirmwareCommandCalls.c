...
static inline error_t _checkResponseForErrors(uint8_t minLength, uint8_t opcode, uint8_t cri, const IfxRfe_ctrxResponse_t *rsp);
...
error_t IfxRfe_fwCall_executeDirectly(uint8_t command_id, IfxRfe_ctrxRequest_t *cmd, IfxRfe_ctrxResponse_t *rsp)
{
    ...
    IFXRFE_RETURN_ON_ERROR(_checkResponseForErrors(EXECUTE_DIRECTLY_MIN_LENGTH, GET_OPCODE(cmd->buffer[0]), GET_CRI(cmd->buffer[0]), rsp));
...
error_t IfxRfe_fwCall_executeDirectly_read(IfxRfe_asyncHandle_t handle, IfxRfe_ctrxResponse_t *rsp)
{
    ...
    IFXRFE_RETURN_ON_ERROR(_checkResponseForErrors(EXECUTE_DIRECTLY_MIN_LENGTH, handle.opCode, handle.CRI, rsp));
...
error_t IfxRfe_fwCall_configureHandle(uint8_t handle_id, IfxRfe_configHandleAction_t action, IfxRfe_ctrxRequest_t *cmd, uint8_t *handle)
{
    ...
    IFXRFE_RETURN_ON_ERROR(_checkResponseForErrors(CONFIGURE_HANDLE_MIN_LENGTH, GET_OPCODE(cmd->buffer[0]), GET_CRI(cmd->buffer[0]), &rsp));
...
error_t IfxRfe_fwCall_configureAndExecuteHandle(uint8_t handle_id, IfxRfe_configHandleAction_t action, IfxRfe_ctrxRequest_t *cmd, uint8_t *handle, IfxRfe_ctrxResponse_t *rsp)
{
    ...
    IFXRFE_RETURN_ON_ERROR(_checkResponseForErrors(CONFIGURE_EXECUTE_HANDLE_MIN_LENGTH, GET_OPCODE(cmd->buffer[0]), GET_CRI(cmd->buffer[0]), rsp));
...
error_t IfxRfe_fwCall_executeHandle(uint8_t handle, uint8_t *command_id, IfxRfe_ctrxResponse_t *rsp)
{
    ...
    IFXRFE_RETURN_ON_ERROR(_checkResponseForErrors(EXECUTE_HANDLE_MIN_LENGTH, GET_OPCODE(cmd.buffer[0]), GET_CRI(cmd.buffer[0]), rsp));
...
static inline error_t _checkResponseForErrors(uint8_t minLength, uint8_t opcode, uint8_t cri, const IfxRfe_ctrxResponse_t *rsp)
{
    ...
    if (_errorCheckFlags & ERROR_CHECK_FLAG_CRI)
    {
        //check CRI
        if (cri != GET_CRI(rsp->buffer[0]))
        {
