...
Logging is currently implemented just using _printf_. The iRFE driver suggest this should be implemented in an async way, which is currently not the case.
...
SPI is implemented using the _spidev_ Linux driver to access the bus from userspace. A speciality here is that chip-selects are, where possible, controlled as GPIO pins. This is required as the iRFE driver requires the processing of responses within one SPI frame (i.e.: keeping CS asserted). This can be seen in the code snippet [here](https://bitbucket-dmz.infineon.com/projects/MIMORAD/repos/ctrx-irfe-driver/browse/src/IfxRfe_Spi.c?at=refs%2Fheads%2Fdevelop#52-83). The Linux driver although treats each transaction (read, write or both) as atomic operation and does not to keep CS asserted afterwards. Hence, the workaround was to control the CS manually as GPIO.
