...
{
    return PlatformI2c_readRegister(gI2cAddress, reg_start + port, 1, value);
}

static inline int TCA9539_setRegister(uint8_t const reg_start, uint8_t const port, uint8_t const value)
{
    return PlatformI2c_writeRegister(gI2cAddress, reg_start + port, 1, &value);
}
